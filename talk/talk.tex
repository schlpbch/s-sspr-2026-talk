...
\begin{frame}[plain]
  ...
  \vspace{0.85cm}
	{\usebeamerfont{author}\usebeamercolor[fg]{author}Andreas Schlapbach (schlpbch@asc.engineering)\par}
  \vspace{0.10cm}
...
\begin{frame}{Kernel PCA turns 28 Years Old} 
  \begin{itemize}
    ...
    \item{But {\color{ascTeal600}How do you certify that a dataset contains no kernel?}}
...
\begin{frame}{The Paper Under Audit}
   \begin{itemize}
      \item \emph{Renold et al.: "Methodological Framework for a Deeper Understanding of Airline Profit Cycles in the Context of Disruptive Exogenous Impact", Journal of Air Transport Management, 2023}.
      ...
      \item They perform \kmeans{} with $k=6$, PCA and system-dynamics modelling.
...
\begin{frame}{Two Risks Left Unexamined}
   \begin{enumerate}
    ...
    \end{enumerate}
    \vspace{0.15cm}
     \muted{If either is material, the regime labels claim cannot be taken at face value.}
\end{frame}
...
\begin{frame}{Enter the Kernels: the Certificate's Core Idea}
  ...
  \begin{itemize}
    ...
    \item Three families fail for three \emph{different} reasons. Their agreement is evidence.
  \end{itemize}
\end{frame}
...
\begin{frame}{The Supervised Complement: Kernel Ridge Regression}
	Predict operating profit from the six other raw variables using \teal{kernel ridge regression} (LOOCV, nested grid search for $\gamma, \alpha$).
  \vspace{0.3cm}
  \begin{center}
  \small
  \begin{tabular}{@{}l r r@{}}
...
\begin{frame}{The Verdict on the Paper}
  \begin{itemize}
    ...
    \item[{\color{ascSlate400}$\circ$}] \textbf{Practical recommendation:}
      ...
      \emph{suppresses} the model-selection.
      signal.
  \end{itemize}
\end{frame}
...
\begin{frame}{The Linearity Certificate, as a Check List}
  ...
  \begin{enumerate}
    \item \textbf{Measure Effective Dimensionality} via participation ratio of
