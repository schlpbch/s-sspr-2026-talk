...
\begin{frame}[plain]
  ...
  \vspace{0.85cm}
	{\usebeamerfont{author}\usebeamercolor[fg]{author}\teal{Andreas Schlapbach} (schlpbch@asc.engineering)\par}
  \vspace{0.10cm}
...
\begin{frame}{The Paper Under Audit}
   \begin{itemize}
      \item Renold et al.: \emph{Methodological Framework for a Deeper Understanding 
...
\begin{frame}{Future Work}
  ...
  \begin{itemize}
    \item \textbf{Small-sample Rigor First.} Exact permutation tests on the 
      silhouette peak and bootstrapped \pc{} loadings (sign-aligned) turn Findings II--IV from suggestive into certified.
    \item \textbf{Temporal Dynamics, Constrained.} With such few data, a Gaussian
       HMM already over-parameterizes. Only Markov-switching Vector Auto Regression (VAR) with strong priors can be entertained.
    \item \textbf{Representation-learning Baselines, as Supplement not Proof.}
      $t$-SNE/UMAP geometry is too perplexity-sensitive at $n=26$ to certify
      anything; autoencoders would simply memorize.
    \item \textbf{Applying our Methodology to Other Transport Sectors.}
      \begin{itemize}
	 \item Can we show that the collinearity--silhouette-suppression pattern is domain-general?
	 \item  Is it applicable to rail/public transport?
     \end{itemize}
  \end{itemize}
\end{frame}
