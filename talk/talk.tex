...
\author{Andreas Schlapbach(schlpbch@asc.engineering)}
...
\begin{frame}[plain]
  ...
  \vspace{0.30cm}
  {\large{The linearity certificate: How to show a dataset has no kernel.}\par}
  \vspace{0.85cm}
	{\usebeamerfont{author}\usebeamercolor[fg]{author}\teal{Andreas Schlapbach} (schlpbch@gmail.com)\par}
  \vspace{0.10cm}
...
\begin{frame}{Kernel PCA turns 28 Years Old} 
  \begin{itemize}
    \item{The kernel methods turn 28 age this year:  \textbf{Sch\"olkopf, Smola \& M\"uller, \emph{Nonlinear component analysis as a kernel eigenvalue problem}, Neural Computation, 1998.}}

    \item{But \teal{How do you certify that a dataset contains no kernel?}}
...
\begin{frame}{The Paper Under Audit}
   \begin{itemize}
      \item Renold et al.: \textbf{Methodological Framework for a Deeper Understanding 
	  of Airline Profit Cycles in the Context of Disruptive Exogenous Impact}, 
          Journal of Air Transport Management, 2023.
      \item Studies US airline profit cycles between 1995 and 2020.
      \item Data set consists of seven variables: \emph{operating profit}, \emph{fuel},
	      \emph{wages}, \emph{revenue per mile}, \emph{load factor},
		\emph{yield}, and \emph{other expenses}.
	\item They perform \teal{\kmeans{} clustering with $k=6$, PCA} and system-dynamics modelling.
      \item \emphf{Headline claim:} COVID-19 does \emph{not} permanently disrupt 
          airline profit regimes: post-pandemic patterns revert to pre-COVID clusters.
    \end{itemize}
\end{frame}

\begin{frame}{Two Risks Left Unexamined}
   \begin{enumerate}
      \item The seven inputs are \teal{highly collinear}: secular growth drives them all. 
...
\begin{frame}{Seven Variables, 2.46 Effective Dimensions}
  ...
  \vspace{0.25cm}
  \begin{itemize}
    \item Of seven nominal dimensions, 2.46 are real: \teal{severe
...
\begin{frame}{PCA: The Three Spaces}
...
\begin{frame}{Finding I: Assignments Are Invariant Across Spaces}
  \begin{columns}[T]
    ...
    \begin{column}{0.4\textwidth}
      \begin{itemize}
        \item The \textbf{Adjusted Rand Index (ARI)} is a chance-corrected version of the Rand Index, which counts pairs of points agreeing (both together or both separate) across two partitions.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Recapitulation of Silhouette Coefficient and Davies--Bouldin Index}
  \begin{itemize}
    \item \textbf{Silhouette Coefficient}:
    \[
      \sil = \frac{b - a}{\max(a,b)}
    \]
    where $a$ is mean intra-cluster distance and $b$ is mean distance to the nearest other cluster; averaged over all points for the score. Ranges $-1$ to $1$, higher is better.
    \item \textbf{Davies--Bouldin Index}:
    \[
      \db = \frac{1}{k}\sum_{i=1}^{k} \max_{j\neq i} \frac{s_i + s_j}{d(c_i, c_j)}
    \]
    where $s_i$ is the average distance of points in cluster $i$ to its centroid $c_i$, and $d(c_i, c_j)$ is the distance between centroids $c_i$ and $c_j$. Lower values indicate tighter, better-separated clusters.
  \end{itemize}
\end{frame}
...
\begin{frame}{Enter the Kernels: the Certificate's Core Idea}
  ...
  \begin{columns}[T]
    \begin{column}{0.31\textwidth}
      \begin{exampleblock}{Distance-based}
        \small Radial-basis Function\\[2pt]
  ...
  \begin{itemize}
    ...
    \item All bandwidths by \teal{median heuristics}
  \end{itemize}
\end{frame}
...
\begin{frame}{Reading the Agreement}
  Three different mechanisms, same shift speaks \teal{one voice}.
  \vspace{0.2cm}
  \begin{exampleblock}{The certificate}
    Six kernels from three fundamentally different families agree with each
    other and with linear PCA. \teal{The manifold contains no detectable 
    curvature.} The absence of nonlinear structure is itself the finding.
  \end{exampleblock}
\end{frame}


\begin{frame}{The Supervised Complement: Kernel Ridge Regression}
  Using \teal{Kernel Ridge Regression} to predict operating profit from the six other raw variables.
  \vspace{0.3cm}
  \begin{center}
  \small
  \begin{tabular}{@{}l r r@{}}
  \toprule
  \textbf{Model} & \textbf{$R^2$ (excl.\ 2020)} & \textbf{$R^2$ (all years)} \\
  \midrule
  Linear ridge                 & 0.831          & \textbf{0.811} \\
  Kernel ridge (RBF)           & \textbf{0.836} & 0.592 \\
  Kernel ridge (poly., deg.~2) & 0.380          & 0.380 \\
  \bottomrule
  \end{tabular}
  \end{center}
  \vspace{0.3cm}
  \begin{itemize}
    \item \textbf{2020 excluded:} all three models perfom comparable
      kernel flexibility buys \teal{no predictive advantage}. 
      In-distribution, the manifold is linear.
    \item \textbf{2020 included:} kernels collapse, linear ridge does not.
      That collapse is kernel \emph{extrapolation} failing at a
      genuine outlier and not an evidence of hidden curvature.
  \end{itemize}
\end{frame}


\begin{frame}{The Verdict on the Paper}
  \begin{itemize}
    ...
    \item[{\color{ascSlate400}$\circ$}] \textbf{But the taxonomy is
      ...
      $k=6$ is imposed, defensible for year-by-year policy description, but \emhp{not}
      recommended by the indices.
    \item[{\color{ascSlate400}$\circ$}] \textbf{Practical recommendation:}
      Cluster on \pc{} scores, not raw variables --- the raw space
      \emph{suppresses} the model-selection signal.
  \end{itemize}
\end{frame}
...
\begin{frame}{The Linearity Certificate, as a Check List}
  ...
  \begin{enumerate}
    \item \teal{Measure effective dimensionality} via participation ratio of
      the eigenvalue spectrum. Low \pr{} $\Rightarrow$ expect a near-linear
      manifold and a distorted raw space.
    \item \teal{Cluster on \pc{} scores}, and compare against raw-space
      assignments (\ari{}). Explain any invariance geometrically.
    \item \teal{Scan $k$ in the clean space:} internal indices in raw
      space can be suppressed by the collinear axis.
    \item \teal{Attack with kernel families that fail differently:}
    ...
    \item \teal{Cross-check with supervised kernels:} kernel vs.\ linear
...
\begin{frame}[plain]
  ...
  Kernel PCA works as the audit tool for its own linear ancestor.

\end{frame}


\begin{frame}{Future Work}
  Restraining the work to small sample sizes, the following directions are promising:

  \begin{itemize}
    \item \textbf{Temporal Dynamics, Constrained.} With such few data, a Gaussian
       HMM already over-parameterizes. Markov-switching Vector Auto Regression (VAR) with strong priors could be explored.
    \item \textbf{Representation-learning Baselines, as Supplement not Proof.}
      $t$-Distributed Stochastic Neighbor Embedding ()$t$-SNE)/Uniform Manifold Approximation and Projection (UMAP) geometry is too perplexity-sensitive at $n=26$ to certify anything; autoencoders would simply memorize.
    \item \textbf{Applying our Methodology to Other Transport Sectors, e.g. public transport.} Hint: rail is less homogeneous than air.
  \end{itemize}
\end{frame}
...
\begin{frame}{Backup: Data}
  \begin{figure}[htbp]
    \centering
    {\includegraphics[alt={Renolds data}, height=65mm]{figures/data-renold.png}}
  \end{figure}
\end{frame}
